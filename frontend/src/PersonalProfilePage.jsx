...
const StatusPill = ({ value }) => (
  <span className="rounded-full bg-emerald-100 px-3 py-1 text-xs font-bold text-emerald-700">{value}</span>
);

const PersonalProfilePage = ({ activeView = 'profile', onChangeView = () => {} }) => (
  <AppFrame
  ...
  >
    <div className="grid gap-6 xl:grid-cols-[0.8fr_1.2fr]">
      <section className="space-y-5">
        <div className="rounded-[28px] border border-[#dce8f5] bg-white p-6 shadow-sm">
      ...
      <section className="rounded-[28px] border border-[#dce8f5] bg-white p-6 shadow-sm">
        <div className="flex flex-col gap-4 border-b border-[#e7eff8] pb-5 lg:flex-row lg:items-center lg:justify-between">
          <div>
            ...
            <p className="mt-2 text-sm text-slate-500">
              Các trường được điền sẵn theo dữ liệu hiện tại để người dùng sửa trực tiếp.
            </p>
          </div>
        ...
        <label className="mt-4 block">
          ...
          <textarea rows="4" className="w-full rounded-[24px] border border-[#dce8f5] px-4 py-3 outline-none focus:border-[#1f5dcc]" defaultValue="Ký túc xá phía Tây, Đại học Đà Nẵng" />
        </label>
        ...
        <div className="mt-6 flex flex-wrap gap-3">
          ...
          <button className="rounded-2xl border border-[#dce8f5] bg-white px-5 py-3 font-semibold text-slate-600">
            Quay lại hồ sơ
          </button>
        </div>
      </section>
