...
export default function LienChiEditEventPage() {
  ...
  useEffect(() => {
    const fetchEvent = async () => {
      try {
        ...
        if (event.timelines) {
          setTimelineItems(event.timelines.map(t => ({ dateTime: formatDateTime(t.dateTime), description: t.description })));
        }
  ...
  const handleSubmitEvent = async () => {
    ...
    try {
      ...
      const data = await response.json();
      setNotice(`✅ Cập nhật sự kiện thành công! Chờ chuyển hướng...`);
      setTimeout(() => {
  ...
  return (
    <LienChiLayout
      ...
      title="Sửa sự kiện toàn diện"
      subtitle="Chỉnh sửa mọi thông tin và gửi yêu cầu duyệt lại."
    >
          ...
          <motion.div whileHover={{ y: -3 }} className="profile-panel rounded-[28px] border border-[#dce8f5] bg-white p-5">
            ...
            {existingImages.length > 0 && (
              <div className="mt-3 space-y-2">
                <p className="text-xs font-semibold text-slate-500">Ảnh đã có (Nếu upload ảnh mới, ảnh cũ sẽ bị xóa hết):</p>
                {existingImages.map((img, idx) => (
                  <div key={`existing-${idx}`} className="flex items-center justify-between rounded-lg bg-slate-50 p-2">
                    <span className="text-xs text-slate-600">{img.imageUrl}</span>
                  </div>
                ))}
              </div>
            )}
          ...
          <motion.div whileHover={{ y: -3 }} className="profile-panel rounded-[28px] border border-[#dce8f5] bg-white p-5">
            ...
            <div className="mt-4 space-y-3">
              {timelineItems.map((item, idx) => (
                <div key={idx} className="flex flex-wrap items-center gap-3 rounded-2xl bg-[#f6faff] px-4 py-3 text-sm text-slate-600">
                  <input
                    type="datetime-local"
                    value={item.dateTime}
                    onChange={(e) => {
                      const newItems = [...timelineItems];
                      newItems[idx].dateTime = e.target.value;
                      setTimelineItems(newItems);
                    }}
                    className="rounded-xl border border-[#dce8f5] px-3 py-2 outline-none focus:border-[#1f5dcc]"
                  />
                  <input
                    type="text"
                    ...
                    className="flex-1 rounded-xl border border-[#dce8f5] px-3 py-2 outline-none focus:border-[#1f5dcc] min-w-[200px]"
                  />
                  <button type="button" onClick={() => handleRemoveTimeline(idx)} className="text-red-500 hover:text-red-700 p-2">
                    <X className="h-4 w-4" />
                  </button>
                </div>
              ))}
