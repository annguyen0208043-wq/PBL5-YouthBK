...
const menuItems = [
  ...
  { id: 'events', label: 'Sự kiện của tôi' },
...
function UserIdentity({ user, subtitle }) {
  ...
  return (
    <div className="profile-user-chip rounded-3xl bg-white/10 p-4 backdrop-blur-md">
...
function HeaderIdentity({ user }) {
  ...
  return (
    <div className="profile-header-user rounded-3xl border border-[#dce8f5] bg-[#f7fbff] px-4 py-3">
      <div className="flex items-center gap-3">
        {user.avatarUrl ? (
...
function ProfileLayout({ activeView, onChangeView, children, title, subtitle, role, user, notificationCount = 0 }) {
  return (
    <div className="profile-page p-4 sm:p-6">
      <div className="profile-shell profile-card mx-auto flex w-full max-w-375 overflow-hidden rounded-4xl border border-[#d8e7f5] bg-[#f8fbfe]">
        <aside className="hidden w-70 border-r border-[#dce9f6] bg-[linear-gradient(180deg,#113b90_0%,#1958c2_100%)] px-5 py-6 text-white lg:flex lg:flex-col">
          ...
          <nav className="space-y-2">
            {menuItems.map((item) => (
              ...
              >
                ...
                {item.id === 'notifications' && notificationCount > 0 ? <Badge count={notificationCount} /> : null}
              </button>
            ))}
          </nav>
